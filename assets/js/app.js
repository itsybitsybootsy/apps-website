...
(function () {
  ...
  /* ---- reviews marquee (both card sets are static in the HTML; JS tunes speed + Safari-safe shift) ---- */
  document.querySelectorAll('[data-rev-marquee]').forEach(function (wrap){
    ...
    function setPlaying(){
      wrap.classList.toggle('is-visible', pageVisible && inView);
    }

    function setLoopWidth(el){
      var cards = el.querySelectorAll('.review');
      if (!cards.length) return 0;
      var last = cards[cards.length - 1];
      return last.offsetLeft + last.offsetWidth + 18;
    }

    function measure(){
      var setWidth = Math.round(setLoopWidth(set));
      if (!setWidth) return;
      track.style.setProperty('--rev-shift', setWidth + 'px');
      track.style.setProperty('--rev-dur', (setWidth / speed) + 's');
    ...
    if ('IntersectionObserver' in window){
      var io = new IntersectionObserver(function (entries){
        entries.forEach(function (e){
          if (e.target === wrap) inView = e.isIntersecting;
        });
        setPlaying();
